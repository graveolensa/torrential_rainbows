...
__device__ cuComplex mervana(cuComplex z, cuComplex ao, cuComplex uo)
{
int v;
cuComplex unity(1.0,0.0);
cuComplex out(1.0,0.0);
cuComplex tle(1.0,0.0);
cuComplex rhuva(3.0,0);
cuComplex netha(1.0,0.0);
cuComplex arvir(0.96592582628906831,0.25881904510252074 );
for(v=0;v<5;v++)
    {
      
        out= out * expc(ao/(ao-uo*powc(z,powc(rhuva,tle)))-ao);
        tle=tle+unity;
...
__device__ cuComplex marvana(cuComplex z, cuComplex ao, cuComplex uo)
{
cuComplex out(0.0,0.0);
cuComplex tiny(0.001,0.001);
out=(mervana(z+tiny,ao,uo)-mervana(z,ao,uo))/tiny;
return out;
}

__device__ cuComplex arvana(cuComplex z, cuComplex ao, cuComplex uo)
{
cuComplex out(0.0,0.0);
cuComplex tiny(0.001,0.001);
out=(marvana(z+tiny,ao,uo)-marvana(z,ao,uo))/tiny;
return out;
}
...
for(v=0;v<20;v++)
    {
        ...
        accume= accume * expc(aon/(uon+powc(q,powc(rhuva,tle))));
        tle=tle+unity;
...
// for the lines of the hyperbolic tiling, we want the second derivative of mervana, which I'm going to define above.
