...
for(v=0;v<5;v++)
{
    cue = (cue - uon*conj(aon-hilva(cue)))/(cue + aon*conj(uon*helva(cue)));
cue = (cue + uon*conj(aon-hilva(cue)))/(cue -aon*conj(uon*helva(cue)));
cue = (cue - uon*conj(aon-hilva(cue)));
       accume = accume + aon*conj(uon*helva(cue));
...
for(v=0;v<5;v++)
{
    cue = (cue - uon*conj(aon-hilva(cue)))/(cue + aon*conj(uon*helva(cue)));
cue = (cue + uon*conj(aon-hilva(cue)));
cue = (cue - uon*conj(aon-halva(cue)));
       accume = conj(accume + aon*conj(uon*helva(cue)));
        /*accume = accume + urigo(powc(cue * aon - conj(cue*uon),aon),uon*fixon);*/
}
accume = uon*the3(q,flat(accume)*fixon*aon)/powc(conj(the3(q,flat(accume)*faxon*uon)),uon);
