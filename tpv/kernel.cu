...
for(v=-4;v<floor(-4+8*moux.r);v++)
{

cue = cue - powc(hilva(cue)/ (hilva(cue+hilva(cue))/hilva(cue)-unity),aon*powc(helvp(cue)/ (helvp(cue+helvp(cue))/helvp(cue)-unity),uon));
accume = accume - hilva(cue+aon*hilva(cue));
}
...
for(v=-4;v<floor(-4+8*mouy.i);v++)
{

cue = cue - powc(hilva(cue)/ (hilva(cue+hilva(cue))/hilva(cue)-unity),uon*powc(hinva(cue)/ (hinva(cue+hinva(cue))/hinva(cue)-unity),aon));
accume = accume +hinva(cue+uon*ai*hinva(cue));
}

cue = cue/nue;

cue = the3(cue,flat(cue)*fixon) - accume;
