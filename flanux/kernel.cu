...
void distanceKernel(uchar4 *d_out, int w, int h, int2 pos) {
  ...
  cuComplex ip(pi,0.0);
    ...
    float fx = 10-scale * (float)(DIM/2 - c)/(DIM/2);
    float fy = 10+ scale * (float)(DIM/2 - r)/(DIM/2);
...
/*while((v<100)&&norg(cue)<2.0)
{
   ...
   for(v=0;v<10;v++)
   {
cue = cue -arago(cue*irigo(cue,uon*faxon),aon*fixon);
accume = accume * uon*arago(cue,faxon);
}
cue = accume;
