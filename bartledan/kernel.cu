...
/*while((v<100)&&norg(cue)<2.0)
{
   ...
   {
/*    gloon = gloon * tle;
    lue = expc(moux/(moux-powc(cue,powc(gloon,tle))));
    vlarv = vlarv + tle;
accume = accume * lue;*/
/*        cue = (unity-cue +cue*cue*cue)/((unity-cue-cue*cue)*(cue-unity)*(cue-unity));
        accume = accume + mouse*tle *cue;
        tle = vro*mouse*tle;
   }*/

        cue =  cue - powc((cosc(sins(moux*cue))-sins(cosc(mouy*cue))-cue),mouse)/(sins(cue)*cosc(mouy*cosc(cue)) - cosc(cue)*sins(moux*sins(cue)) - unity);
                accume = accume + cue;
