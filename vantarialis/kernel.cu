...
for(v=0;v<10;v++)
    {
    /* here we factor Newton's method:
    z = z - f(z)/f'(z) yields
    (sqrt(z) - i sqrt(f(z)/f'(z)))*  (sqrt(z) - i sqrt(f(z)/f'(z)))
    z starts as q, and then we want to apply a rotation matrix to each part, and
    then multiply them together
    */
       
        vue = powc(cue,mrun);
        lue = powc((aciwa(powc(uon,halva(cue)))-aon*cue)/(hinva(cue)-aon),mrun);
        pnx = aon * vue - ai * uon * lue;
        pny = uon * vue + ai * aon * lue;
        cue = pnx * pny;
        accume = accume * powc(pnx/pny,aon/mrun);
