...
__device__ cuComplex the9(cuComplex z, cuComplex q, cuComplex r)
{
    int v;
    cuComplex unity(1.0,0.0);
    cuComplex out(1.0,0.0);
    ...
    {
        qoo = qoo * q * q;
        out = out * (unity - qoo) * (unity + tw * qoo/q * the3(tw*z,r) + qoo*qoo/(q*q)); 
...
__device__ cuComplex origa(cuComplex z, cuComplex q)
{
    int v;
    cuComplex unity(1.0,0.0);
    cuComplex out(1.0,0.0);
    ...
    {
        qoo = qoo * q * q;
        out = out * (unity - qoo) * (unity + tw * qoo/q * powc(aciwa(q*z),aciwa(q*z)) + qoo*qoo/(q*q)); 
...
__device__ cuComplex arth(cuComplex z, cuComplex q, cuComplex ao, cuComplex uo, cuComplex fic)
{
    int v; 
    cuComplex out=z;
    cuComplex lue(1.0,0.0);
    for(v=0;v<10;v++)
    {
    out = out - lanna(ao/(uo-lue))/hinva(uo/(ao+lue));
    lue = lue * q;   
}
return out;
}
...
cuComplex pnx(0.0,0.0);
cuComplex pny(0.0,0.0);
cuComplex mx = moux;
cuComplex my = mouy;
cuComplex lx=pnx;
cuComplex ly=pny;
...
//arth(cuComplex z, cuComplex q, cuComplex ao, cuComplex uo, cuComplex fic)

// For Questular, Let's see what happens with one of Ramanujan's mock theta functions?



//cue = thy(q,flat(accume)*fixon);

for(v=0;v<6;v++)
{
    mx = mx - (hilva(aon*ly)-uon*mx)/(uon*aciwa(aon*mx)-aon);
    lx = lx + (hilva(aon*q)-uon*mx);
    my = my - (halva(aon*mx)-uon*my)/(uon*aciwa(aon*my)-aon);
    ly = ly + (halva(uon*q)-aon*my);
    if(norg(cue)>1)
    {
        cue = conj(unity/cue);
    } 
    cue = cue - lx*(rampsi(aon*cue)-uon*ly*cue)/(uon*ramchi(aon*cue)-aon*lx);
}
