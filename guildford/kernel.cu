...
void distanceKernel(uchar4 *d_out, int w, int h, int2 pos) {
  ...
  cuComplex ip(pi,0.0);
    ...
    float fx = -scale * (float)(DIM/2 - c)/(DIM/2)+50;
    float fy = scale * (float)(DIM/2 - r)/(DIM/2)+50;
...
cue = cue - aon*arago(cue,fixon*flat(cue));
accume = accume + uon*hilva(cue-aon*alvir(cue));
}
