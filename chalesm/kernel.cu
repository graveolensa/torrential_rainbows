...
__device__ cuComplex hulva(cuComplex z)
{
    cuComplex ree(j0f(z.r),0.0);
    cuComplex out(cosf(j1f(z.r)),sinf(j1f(z.i)));
    out = out * ree;
    return out;
}
...
__device__ cuComplex arago(cuComplex z, cuComplex q)
{
    ...
    {
        qoo = qoo * q * q;
        out = out * (unity - qoo) * (unity + tw * qoo/q * hulva(tw*z) + qoo*qoo/(q*q)); 
...
cue = powc(hulva(cue*aon),hulva(cue*uon));
