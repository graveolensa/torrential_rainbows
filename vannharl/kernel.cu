...
for(v=0;v<10;v++)
    {
	nue = cue;
    cue = (cue - aon* powc(hilva(cue),moeb(aon,fixon,halva(cue))))/(cue - uon* powc(halva(cue),hilva(cue)));
    accume = accume *(unity - cue)/(unity + nue)* expc(cue/nue);
...
for(v=0;v<10;v++)
    {
    nue = cue;
    cue = (cue + aon* powc(cosc(cue),moeb(aon,faxon,halva(cue))))/(cue - uon* powc(moeb(aon,fixon,hilva(cue)),halva(cue)));
    accume = accume *(unity - cue)/(unity + aon*nue);
