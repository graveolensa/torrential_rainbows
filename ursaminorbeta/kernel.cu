...
for(v=0;v<50;v++)
{
   ...
   // cue = moux * powc(the2(cue,uon*fixon)*the4(cue,aon*faxon),gloon)/powc(the3(cue,uon*aon*fixon),gloon+gloon);
    //Plot3D[Sum[Sin[Pi k^a x]/(Pi k^a), {k, 1, 50}], {x, 0, 1}, {a, 2, 3}, PlotPoints -> 100]

accume = accume + sins(ip *powc(kei,effx*effy*mouy))/(ip * powc(kei,effy*mouy)) + sins(ip *powc(kei,effx*effy*moux))/(ip * powc(kei,effx*moux)); 
kei = kei+unity;
}
cue = accume;
