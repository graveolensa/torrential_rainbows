...
for(v=0;v<10;v++)
{
	cue = cue -aon*conj(aon*hilva(cue)-conj(uon*ai*hinva(cue)))/(uon*hilva(cue)+conj(aon*ai*hinva(cue)));
}
...
for(v=0;v<10;v++)
{
    ...
    cue = cue = cue -aon*conj(aon*hilva(cue)-conj(uon*ai*hinva(cue)))/(uon*hilva(cue)+conj(aon*ai*hinva(cue)));
}
    cha = ((atan2(cue.i,cue.r) - pi)/(2.0*pi));
    amp = cha + 1.0;

     d_out[i].x = (unsigned char) (amp*255.0*pow(sin(pi*tha),2));
    d_out[i].y = (unsigned char) (amp*255.0*pow(sin(pi*tha+pi/3),2));
    d_out[i].z  = (unsigned char) (amp*255.0*pow(sin(pi*tha+2*pi/3),2));
    d_out[i].w = 0;
...
__global__
void audioKernel(double *out, int w, int h, int2 pos) {
  const int c = blockIdx.x*blockDim.x + threadIdx.x;
  const int r= blockIdx.y*blockDim.y + threadIdx.y;
  const int i = c + r*w; // 1D indexing
  float pi = 3.1415926535898;
...
for(v=0;v<10;v++)
{
   	nue = aon*conj(aon*hilva(cue-conj(uon*ai*hinva(cue))));
    cue = cue -nue/(uon*hilva(cue)+conj(aon*ai*hinva(cue)));
    if(v==5)
    {
    	out[i] = 2048*cue.i-12800*cue.r;
    }
}
...
if(v % i ==7)
    {
    	for(u=0;u<10;u++)
    	{
    	out[i] = cosf((aon.r*out[i] - 10294*cue.r)/(out[i]+ 2048*cue.i));
    	}
    }
...
void audioKernelLauncher(double* audio_buffer, int len, int w, int h, int2 pos) {
  ...
  long size = len*sizeof(double)*2;

  cudaMalloc((void **)&d_audio_buffer, size);
  audioKernel<<<len, 1>>>(d_audio_buffer, w, h, pos);

  cudaMemcpy(audio_buffer, d_audio_buffer, size, cudaMemcpyDeviceToHost);
  cudaFree(d_audio_buffer);
