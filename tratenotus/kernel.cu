...
for(v=0;v<40;v++)
    {
        cue = cue - (expc(hilva(cue)*aon - ai*hinva(cue)*uon))/(hilva(cue)*aon - ai*hinva(cue)*uon);
        if(v %5==0)
        {
        accume = powc(aon,accume) * (expc(hilva(cue)*aon - ai*hinva(cue)*uon));
    }
    else
    {
        accume = powc(accume,uon) * (expc(hilva(cue)*uon + ai*hinva(cue)*aon));
    }
}
