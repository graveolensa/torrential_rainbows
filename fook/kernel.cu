...
cue - cue - (aon*helva(cue)-uon*alvir(cue))/(aon*hilva(cue)-uon);
accume = accume +helva(cue);
}

cue = the3(accume,flat(cue)*fixon*aon);
...
cue = origo(q,moeb(flat(accume)*fixon,uon,cue));
