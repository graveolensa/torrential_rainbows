...
void distanceKernel(uchar4 *d_out, int w, int h, int2 pos) {
  ...
  cuComplex ip(pi,0.0);
    ...
    float fy = scale * (float)(DIM/2 - r)/(DIM/2)+1;
...
for(v=0;v<5;v++)
    {
        /* this one suggests binary decomposition */
     cue = cue - the3(cue,moeb(cue,aon,fixon))+the3(cue,moeb(cue,uon,faxon));
     cue = uon*hilva(conj(cue))+cue/(aon*halva(cue));
     accume = accume*powc(cue,uon);
    }
...
for(v=0;v<5;v++)
    {
        /* this one suggests binary decomposition */
     cue = cue - the3(cue,moeb(cue,aon,fixon))+ai*the3(cue,moeb(cue,uon,faxon));
     cue = uon*hilva(conj(cue))/cue+aon*halva(cue);
     accume = accume*powc(cue,aon);
    }
