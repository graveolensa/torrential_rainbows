...
for(v=0;v<2;v++)
{
    cue = hilva(aon*alvir(cue)-uon*halvi(cue))/hinva(uon*alvir(cue)-aon*halvi(cue));
    nue = nue - hilva(aon*alvir(cue)-uon*halvi(nue))/hinva(uon*alvir(nue)-aon*halvi(nue));
accume = accume * nue * (unity-cue);
}

cue = hilva(conj(cue - nue));
for(v=0;v<2;v++)
{
    cue = hilva(aon*alvir(cue)-uon*halvi(cue))/hinva(uon*alvir(cue)-aon*halvi(cue));
    nue = nue - hilva(aon*alvir(cue)-uon*halvi(nue))/hinva(uon*alvir(nue)-aon*halvi(nue));
