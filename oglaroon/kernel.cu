...
for(v=0;v<30;v++)
{
    ...
    cue = cue + (cosc( powc(D*cue,mrun))-moux*cue)/(sins(powc(D*cue,mrun))/(tir*powc(D/cue,mrun)) + ai*mouy);
    accume = accume + cue;
    nue = nue + (cosc(nue)-mouy*nue)/(sins(nue)-ai*moux*nue);
    eccume = eccume + nue;

    if(cue.r<nue.i)
    {
   cue = cue + (cosc( powc(D*cue,mrun))-moux*cue)/(sins(powc(D*cue,mrun))/(tir*powc(D/cue,mrun)) + moux);
    accume = accume + cue;
    nue = nue + (sins(nue)-mouy*nue)/(sins(cue)-ai*moux*nue);
    eccume = eccume + nue;
    }
    else
    {
    ...
    eccume = eccume + nue;
        if(cue.i>nue.r)
    {
   cue = cue + (cosc( powc(D*cue,mrun))-mouy*cue)/(sins(powc(D*cue,mrun))/(tir*powc(D/cue,mrun)) + moux);
    accume = accume -mouy* cue;
    nue = nue -mouy* (cosc(nue)-mouy*nue)/(sins(nue)-ai*moux*nue);
    eccume = eccume + nue;
    }
    else
    {
    cue = cue - (cosc( powc(D*cue,mrun))-moux*cue)/(sins(powc(D*cue,mrun))/(tir*powc(D/cue,mrun)) + moux);
    accume = accume + cue * (unity-nue);
    nue = nue + (cosc(nue)-mouy*nue)/(sins(nue)-ai*moux*nue);
    eccume = eccume + nue;
    }
    }
