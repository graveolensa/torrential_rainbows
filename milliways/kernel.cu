...
cue = cue - aon *conj(alvir(cue))/alvir(conj(cue)) - uon *powc(ai,tle)*hilva(conj(cue))/conj(hilva(cue))+ helva(conj(cue))/conj(helva(cue));
tle = vro*tle;
}

cue = flat(cue);
cue = the3(cue,flat(cue)*fixon)/the3(cue,flat(cue)*faxon);

for(v=1;v<10;v++)
{

cue = cue - aon *conj(alvir(cue))/alvir(conj(cue)) - uon *hilva(conj(cue))/conj(hilva(cue))+ helva(conj(cue))/conj(helva(cue));
