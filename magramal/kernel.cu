...
__device__ cuComplex hanvara(cuComplex z)
{
    cuComplex out(cyl_bessel_i0f(z.r),cyl_bessel_i1f(z.i));
    return  out;
}

__device__ cuComplex hara(cuComplex z)
{
    cuComplex out(tgammaf(z.r),tgammaf(z.i));
    return  out;
}
...
__device__ cuComplex irigo(cuComplex z, cuComplex q)
{
    ...
    {
        qoo = qoo * q * q;
        out = out * (unity - qoo) * (unity + tw * qoo/q * hilva(tw*z) + qoo*qoo/(q*q)); 
...
/*while((v<100)&&norg(cue)<2.0)
{
   ...
   for(v=0;v<20;v++)
   {
cue = cue - hilva(cue)/((uon*hilva(cue+aon*hilva(cue)))/hilva(cue)-unity);
nue = nue - helva(nue)/((uon*helva(nue+aon*helva(nue)))/helva(nue)-unity);
accume = accume + helva(cue)*hilva(nue);
}
