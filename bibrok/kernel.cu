...
{
nue = cue;
cue = cue - (conj(irigo(cue,uon*faxon))-thy(cue,aon*fixon))/(irigo(cue+irigo(cue,uon*fixon),uon*fixon)/(irigo(cue,aon*fixon)-unity));

accume = accume * expc(conj(irigo(cue,aon*faxon)))  * conj(nue/cue);
