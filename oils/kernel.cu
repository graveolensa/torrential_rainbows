...
cuComplex vee(2.0,0.0);
int v;
for(v=0;v<10;v++)
{
lav = lav - (sins(mel*lav)-rel*lav)/(mel*cosc(mel*lav)-rel);
out = out + (sins(mel*lav)-rel*lav);
}
...
cuComplex ruga(0.5,54.0);
cuComplex senna(0.5,0.0);
...
for(v=0;v<10;v++)
{
    ...
    cue = cue - moeb(aon,trev(cue,aon,uon)/((trev(cue+finch,aon,uon)-trev(cue,aon,uon))/finch),cue);
