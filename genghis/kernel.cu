...
void distanceKernel(uchar4 *d_out, int w, int h, int2 pos) {
  ...
  cuComplex ip(pi,0.0);
    ...
    float fx = 0.5 + -scale * (float)(DIM/2 - c)/(DIM/2);
    float fy = 0.0 + -scale * (float)(DIM/2 - r)/(DIM/2);
...
cuComplex radr = cue;
cuComplex radl = cue;
/* explicitly define a sixth root of unity */
cuComplex sru(0.5,0.86602540378443);
...
/* split into three regions */
 /* moux splits the unit interval into moux, unity -moux , and these should be multiplied by a positive and a negative sixth root of unity */
...
cenr = moux + (unity-moux)*mrun;
radr = (unity-moux/mrun)*mrun;
cenl = moux;
radl = moux*mrun;

if(norg(cue-cenl)<norg(radl))
{
	cue = conj(sru);
}
else
{
	if(norg(cue-cenr)<norg(radr))
	{
		cue = sru;
	}
	else
	{
		if(norg(cue-mrun)<norg(mrun))
		{
		cue = unity;
		}
	}
