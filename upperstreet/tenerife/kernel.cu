...
{
    /* this is a fake -- or poorly truncated polylogarithm function, for examining 
        polylogarithm parameter bivouacks */
    cuComplex out(0.0,0.0);
    cuComplex onu(1.0,0.0);
    cuComplex nn(1.0,0.0);
    cuComplex zz=z;
    int n;
    for(n=1;n<20;n++)
    {
        out = out + powc(z,nn)/powc(nn,s);
        nn = nn + onu;
    }
    return out;
}
...
for(v=0;v<15;v++)
    {
      cue = cue - aon*polylog(q,uon*cue);
      accume = cue * hilva(cue*flat(polylog(cue,q)));
