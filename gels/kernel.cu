...
for(v=0;v<3;v++)
{
lav = lav - rel*(cosq(lav,rel)-powc(V,rel))/cosq(lav,rel);
out = out + mel*(cosq(lav,rel)-powc(V,mel));
}
...
cuComplex nara(-1/2.0,sqrtf(3.0)/2.0);
...
/*fixon = mob(fixon,mouse);
for(v=0;v<10;v++)
{
cue = cue = cue - conj(asins(mob(cue,mouse))/halva(mob(mouse,cue)));
...
for(v=1;v<10;v++)
{
	rue = trev(q,cue,aon*fixon);
    cue = cue - rue/(trev(q,cue+finch,aon*fixon)-rue/finch);
}
