...
__device__ cuComplex urigo(cuComplex z, cuComplex q)
{
    ...
    {
        qoo = qoo * q * q;
        out = out * (unity - qoo) * (unity + tw * qoo/q * (hilva(z)+helva(z)) + qoo*qoo/(q*q)); 
...
/*while((v<100)&&norg(cue)<2.0)
{
   ...
   for(v=0;v<10;v++)
   {
cue = (cue -urigo(cue,uon*faxon))/((cue -powc(ai,uon)*urigo(cue,aon*faxon)));
accume = accume * uon*urigo(cue,aon*faxon);
}
cue = accume;
