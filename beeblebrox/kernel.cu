...
for(v=0;v<10;v++)
{
	cue = aon*sins(cue)-uon*halva(cue);
if(norg(cue)>1)
{
	cue = unity/cue;
    cue = the3(q,fixon*aon*cue);
}
cue = cue - aon*rampsi(cue)/ramchi(cue)-uon*hilva(cue/cosc(cue));
cue = moeb(cue,fixon,aon);
}
